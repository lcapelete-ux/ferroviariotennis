        ...
        {activeTab === 'calendar' ? (
          ...
          <div className="space-y-4">
            {(profile?.role === 'admin' || profile?.canManageChampionships) && (
              <button
                onClick={() => navigate('/admin')}
                className="flex items-center gap-2 px-4 py-2.5 bg-zinc-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-zinc-800 transition-all active:scale-95"
              >
            ...
            {championships.length === 0 ? (
              <div className="py-20 text-center">
                ...
                <p className="text-base font-black text-zinc-400">Nenhum campeonato ativo</p>
                <p className="text-sm text-zinc-400 mt-1">Fique de olho nas novidades!</p>
              </div>
            ) : (
              <div className="space-y-3">
                {championships.map(champ => {
                  ...
                  return (
                    <div key={champ.id} className="bg-white rounded-2xl border border-zinc-100 overflow-hidden">
                      {/* Status + type row */}
                      <div className="flex items-center justify-between px-4 pt-4 pb-2">
                        <span className={clsx(
                          ...
                          isNotStarted ? "bg-blue-100 text-blue-700" : "bg-zinc-100 text-zinc-500"
                        )}>
                          {isRegistrationOpen ? 'Inscrições abertas' : isNotStarted ? 'Em breve' : 'Encerrado'}
                        </span>
                      ...
                      <div className="px-4 pb-3">
                        <h3 className="text-lg font-black text-zinc-900 leading-tight">{champ.title}</h3>
                        {champ.description && (
                          <p className="text-xs text-zinc-500 mt-1 leading-relaxed">{champ.description}</p>
                        )}
                      </div>

                      {/* Info row: dates + fee + deadline */}
                      <div className="mx-4 mb-3 flex items-stretch gap-0 border border-zinc-100 rounded-xl overflow-hidden">
                        ...
                        <div className="flex-1 px-3 py-2.5">
                          <p className="text-[8px] font-black text-zinc-400 uppercase tracking-widest mb-0.5">Taxa</p>
                        ...
                        {myReg ? (
                          <>
                            <div className="flex items-center justify-between py-2.5 px-3 bg-emerald-50 rounded-xl border border-emerald-100">
                              <div className="flex items-center gap-2 text-emerald-700">
                                ...
                                <span className="text-xs font-black">Inscrito</span>
                              </div>
                              {champ.registrationFee && champ.registrationFee > 0 && (
                                <span className={clsx(
                                  "text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full",
                                  myReg.paymentStatus === 'paid' ? "bg-emerald-200 text-emerald-800" :
                                  myReg.paymentStatus === 'informed' ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"
                                )}>
                                  {myReg.paymentStatus === 'paid' ? 'Pago' :
                                   myReg.paymentStatus === 'informed' ? 'Aguardando' : 'Pendente'}
                                </span>
                              )}
                            </div>
                            ...
                            {myReg.paymentStatus === 'pending' && champ.pixKey && (
                              <div className="bg-zinc-50 rounded-xl p-3 space-y-2 border border-zinc-100">
                                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Chave PIX</p>
                                <div className="flex items-center justify-between">
                                  <code className="text-xs font-bold text-zinc-700 truncate mr-2">{champ.pixKey}</code>
                                  <button
                                    onClick={() => { navigator.clipboard.writeText(champ.pixKey || ''); showAlert("Sucesso", "Chave PIX copiada!", "success"); }}
                                    className="text-[10px] font-black text-emerald-600 uppercase shrink-0"
                                  >
                                    Copiar
                                  </button>
                                </div>
                                <button
                                  ...
                                  className="w-full py-2 bg-emerald-600 text-white rounded-lg text-xs font-black uppercase tracking-widest active:scale-95 transition-all"
                                >
                                  Já paguei · Informar
                                </button>
                              </div>
                            ...
                            <div className="flex items-center gap-2">
                              {hasBracket && (
                                <button
                                  ...
                                  className="flex-1 py-2.5 rounded-xl text-xs font-black text-zinc-900 flex items-center justify-center gap-1.5 bg-amber-400 active:scale-95 transition-all"
                                >
                              ...
                              <button
                                ...
                                className={clsx(
                                  "py-2.5 text-xs font-bold text-zinc-400 hover:text-red-600 transition-colors",
                                  hasBracket ? "px-3" : "w-full"
                                )}
                              >
                                Cancelar inscrição
                              </button>
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className={clsx(
                                "flex-1 py-3 rounded-xl font-black text-sm uppercase tracking-widest transition-all active:scale-95",
                                champ.status === 'open' && isRegistrationOpen
                                  ? "bg-emerald-600 text-white shadow-sm"
                                  : "bg-zinc-100 text-zinc-400 cursor-not-allowed"
                              )}
                            >
                              {isNotStarted ? 'Em breve' : isDeadlinePast ? 'Encerrado' : champ.status === 'open' ? 'Inscrever-se' : 'Fechado'}
                            </button>
                            {hasBracket && (
                              <button
                                onClick={() => setViewingBracket(champ.id)}
                                className="px-4 py-3 rounded-xl text-xs font-black text-zinc-900 flex items-center gap-1.5 bg-amber-400 active:scale-95 transition-all"
                              >
                                <Trophy className="w-3.5 h-3.5" />
                                Chaves
                              </button>
